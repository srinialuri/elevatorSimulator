...
public class ElevatorController {
	...
	public void summonElevatorDown(int floorNumber, Person person) {
		...
		while (e == null && person.getKeepRunning()) { // need this to stop
			...
			if (Simulator.debug)
				log.write("" + ++counter
						+ " tries to summon down elevator to floor "
						+ floorNumber);
			e = getSameFloorElevator(floorNumber);
			if (e != null) {
				if (Simulator.debug)
					log.write("Setting down destination for elevator "
							+ e.getElevatorNumber() + " same floor "
							+ floorNumber);
				try {
					e.summonDestination(floorNumber);
					...
					if (Simulator.debug)
						log.write("Moving Exception down1: floor "
								+ floorNumber + " on elevator " + e.getState());
					e = null;
					continue;
				}
			} else {
				if (floorNumber != Building.MAX_FLOORS) { // there won't be any
															// above the top
															// floor
					if (Simulator.debug)
						log.write("looking for one moving down from above to floor "
								+ floorNumber);
				...
				if (e != null) {
					if (Simulator.debug)
						log.write("Setting destination for elevator "
								+ e.getElevatorNumber() + "from  above floor "
								+ floorNumber);
					try {
						e.summonDestination(floorNumber);
					} catch (ElevatorMovingException mx) {
						if (Simulator.debug)
							log.write("Moving Exception down2: floor "
									+ floorNumber + " on elevator "
									+ e.getState());
						e = null;
						continue;
					...
					if (Simulator.debug)
						log.write("Looking for closest stopped elevator for down floor "
								+ floorNumber);
					e = getNearestElevator(floorNumber, Elevator.NO_DIRECTION);
					if (e != null) {
						if (Simulator.debug)
							log.write("Setting destination for stopped elevator "
									+ e.getElevatorNumber()
									+ " for down floor " + floorNumber);
						try {
							e.summonDestination(floorNumber);
						} catch (ElevatorMovingException mx) {
							if (Simulator.debug)
								log.write("Moving Exception down3: floor "
										+ floorNumber + " on elevator "
										+ e.getState());
							e = null;
							continue;
						...
						if (Simulator.debug)
							log.write("Looking for elevator coming up "
									+ floorNumber);
						e = getElevator(floorNumber, Elevator.MOVING_UP);
						if (e != null) {
							if (Simulator.debug)
								log.write("Setting destination for moving up elevator "
										+ e.getElevatorNumber()
										+ " for floor "
										+ floorNumber);
							try {
								...
								if (Simulator.debug)
									log.write("Moving Exception down4: floor "
											+ floorNumber + " on elevator "
											+ e.getState());
								e = null;
								continue;
	...
	public void elevatorArrived(int floorNumber, Elevator elevator) {
		...
		if (direction == Elevator.MOVING_UP && floor.isSummonUp()) {
			...
			floor.elevatorArrivedDown(elevator);
		} else if (floor.getNumberWaitingUp() > floor.getNumberWaitingDown()) { // light
																				// is
																				// off
																				// give
																				// up
	...
	private Elevator getSameFloorElevator(int floorNumber) {
		...
		for (int i = 0; i < elevators.size(); i++) {
			...
			state = e.getState();
			if (e.getCurrentFloorNumber() == floorNumber
					&& state.motionState == Elevator.STOPPED
					&& state.numberRiders == 0) {
				if (Simulator.debug)
					log.write("Summoned elevator " + e.getElevatorNumber());
				break;
			} else
				e = null;
		}
