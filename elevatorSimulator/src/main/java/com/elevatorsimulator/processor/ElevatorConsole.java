...
/**
 * This represent ElevatorConsole
 * @author System
 *
 */
public class ElevatorConsole extends JFrame implements WindowListener {
	
	private static final long serialVersionUID = -2747851210654150618L;
	...
	private void init() {
		...
		timeRemainingLabel = new JLabel(String.valueOf(Simulator
				.getTimeRemaing()));
		...
		constrain(new JLabel(String.valueOf(Building.MAX_ELEVATORS)), 7, -1, 1,
				1);
	...
	private void updateCounters() {
		inBuildingLabel.setText(String.valueOf(Simulator.MAX_PEOPLE
	...
	private void updateElevators() {
		...
		for (int e = 0; e < building.MAX_ELEVATORS; e++) {
			...
			if (elevatorLocations[e] != currentFloor) { // don't redraw if we
														// don't have too
			...
			lCableGrid[e][currentFloor - 1].setText(directionInd
					+ state.numberRiders);
		}
	}
